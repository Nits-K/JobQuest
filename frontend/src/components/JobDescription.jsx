...
const JobDescription = () => {
  ...
  const applyJobHandler = async () => {
    try {
      ...
      if (res.data.success) {
        setIsApplied(true); // Update the local state to reflect the user has applied
        const updatedSingleJob = {
          ...singleJob,
          applications: [...singleJob.applications, { applicant: user?._id }],
        };
        dispatch(setSingleJob(updatedSingleJob)); // Dispatch updated job data to the store
  ...
  return (
    <div className="max-w-7xl mx-auto my-10">
      <div className="flex items-center justify-between">
        ...
        <Button
          onClick={isApplied ? null : applyJobHandler} // Disable the click if already applied
          disabled={isApplied} // Disable button when already applied
